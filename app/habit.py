...
class Habit:
    ...
    def __init__(self, name: str, description: str, periodicity: int):
        ...
        self.best_streak = 0
        self.weekly_completed = [0] * 7
        self.monthly_completed = [0] * 30
    ...
    def track(self):
        ...
        self.weekly_completed[today.weekday()] += 1
        if week_start == self.last_completed:
            week_total = sum(self.weekly_completed)
            if week_total >= 7:
                # The habit has been completed at least once a day for the past week
                print("Weekly completion!")

        # Monthly tracking
        if today.day == 1:
            month_total = sum(self.monthly_completed)
            if month_total >= 30:
                # The habit has been completed at least once a day for the past month
                print("Monthly completion!")
            self.monthly_completed = [0] * 30
        self.monthly_completed[today.day-1] += 1
